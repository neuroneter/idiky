...
| `validacion` | `ValidacionPoder?` | **Solo cuando el propietario envió la foto del papel desde su app** (CU-R-30): `esperando`, `validado` o `rechazado`, con quién decidió, cuándo y el **motivo** si rechazó. Ausente = validado por construcción (RN-79) |
...
| `limiteComision` | fecha ISO? | **Hasta cuándo tiene la comisión para revisar** (RN-78). Lo fija el administrador al designarla; **no puede pasar de `limiteVerificacion`**. Vencido, lo que falte por revisar deja de detener el acta |
...
| RN-77 | **Hay decisiones que una sesión no puede tomar, aunque las vote** (Ley 675 art. 46, parágrafo, verificado el 2026-09-10): *«Las decisiones previstas en este artículo **no podrán tomarse en reuniones no presenciales**, ni en reuniones de segunda convocatoria, salvo que en este último caso se obtenga la mayoría exigida por esta ley»*. Y el mismo artículo cierra: lo adoptado en contravención suya es **absolutamente nulo**. No es un umbral más alto, es una **puerta cerrada**: la copropiedad puede reunirse por Zoom para todo (art. 42, RN-75) menos para esto. **La segunda convocatoria no se bloquea** — la ley la admite si aun así se obtiene el 70 %, que es lo que `resultadoVotacion` ya exige sin relajarlo nunca. **La mixta se trata como no presencial, y eso es una deducción, no una cita**: el art. 46 dice «no presenciales» y en 2001 no existía la mixta; quien la trae al caso es el Decreto 398 de 2020 art. 1 («las reglas relativas a las reuniones no presenciales serán igualmente aplicables a las reuniones mixtas»). Se toma el camino conservador —restringe, no habilita— y queda como pregunta para el abogado (§3 bis). Se comprueba en las dos caras y **en el repositorio**, no solo en el botón (T-16), y el acta lo deja por escrito en vez de reportar una aprobación nula. | `dominio/reglas.ts` (`decisionAdmisibleEnLaSesion`) + `datos/repositorio.ts` (`emitirVoto`) + `componentes/HojaActa.tsx` |
| RN-78 | **La comisión verificadora tiene un plazo máximo, y lo fija el administrador** (Mary, 2026-09-17: *«para la revisión del acta debe existir un plazo máximo que lo define el administrador»*). Sin plazo, un solo miembro que no revise deja el acta en borrador para siempre, y con ella las decisiones de la asamblea: la comisión existe para garantizar el acta, no para bloquearla. El plazo es **por acta, no por reglamento** —se escribe al designar la comisión—, y alrededor de lo que el administrador decide hay dos cosas fijas: **no puede pasar del término del art. 47** (`limiteVerificacion`), porque el acta tiene que estar a disposición en esos veinte días hábiles con o sin revisión, y **con comisión el plazo es obligatorio**: sin él no se aprueba. Mientras corre, el acta espera a todos (RN-76). **Vencido, las revisiones que faltan dejan de detenerla y ya no se registran** —un plazo máximo que admite revisiones después no es máximo—, y **no se borra nada** (RN-61): la hoja dice quién no revisó dentro del plazo y hasta cuándo lo tuvo. El estado se sigue derivando: con el plazo vencido el acta vuelve a `borrador`, no a `en_verificacion`. Se exige **en el repositorio** (T-16), no solo en el `max` del campo de fecha. | `dominio/reglas.ts` (`motivoPlazoComisionInvalido`, `comisionVencida`, `verificadoresQueBloquean`, `verificadoresFueraDePlazo`) + `datos/repositorio.ts` (`designarComisionActa`, `verificarActa`) + `componentes/HojaActa.tsx` |
...
| `codigo` | Lo que la persona escribe, junto con su documento, para abrir su registro y adjuntar (RN-58). Con la marca de abajo, es también **la clave con la que activa su cuenta** (RN-80) |
| `soportesNoObligatorios` | La marca **«No obligatorio»** (RN-80): quién la puso y cuándo. Solo el administrador; solo sobre un registro concreto. Con ella el registro no espera fotos y pasa directo a autorizar |
