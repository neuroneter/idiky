...
| `forma` | `'presencial' \| 'virtual'` | **Pesan igual** (RN-92): la suma de coeficientes es una sola. Se lleva el reparto por separado porque **el acta lo exige** (art. 47), no porque una cuente menos |
...
>
> **Lo virtual pesa lo mismo que lo presencial** (RN-92, Mary 2026-09-10). No es solo una
...
| `registradoPor`, `registradoEn` | string, fecha ISO | El administrador (papel) o el propietario (app). **Darlo de alta es validarlo** en esos dos casos: quien lo hace es quien tiene la potestad |
| `validacion` | `ValidacionPoder?` | **Solo cuando el propietario envió la foto del papel desde su app** (CU-R-31): `esperando`, `validado` o `rechazado`, con quién decidió, cuándo y el **motivo** si rechazó. Ausente = validado por construcción (RN-96) |
...
| `verificadores` | string[] | La **comisión verificadora**, si la hay. **Puede estar vacía**: la ley no la exige (RN-93) |
| `verificaciones` | `VerificacionActa[]` | Quién revisó, cuándo y qué anotó |
| `editadaEn` | fecha ISO? | Existe para una sola cosa: **una revisión vale sobre el texto que se revisó** (RN-93) |
| `limiteVerificacion` | fecha ISO | El término del reglamento y, en su defecto, **20 días hábiles** (art. 47). Copiado al generarla |
| `limiteComision` | fecha ISO? | **Hasta cuándo tiene la comisión para revisar** (RN-95). Lo fija el administrador al designarla; **no puede pasar de `limiteVerificacion`**. Vencido, lo que falte por revisar deja de detener el acta |
...
| RN-74 | **Las dos mayorías se miden sobre bases distintas, y confundirlas anula la votación** (Ley 675, arts. 45 y 46, verificada el 2026-09-10). La **simple** es «la mitad más uno de los coeficientes **representados en la respectiva sesión**»: la base es lo que asistió. La **calificada** es «el setenta por ciento (70 %) de los coeficientes **que integran el edificio**»: aquí la base sí es el edificio entero, y por eso es tan difícil — es a propósito. La calificada **se alcanza** (≥ 70 %); la simple **se supera** (> mitad). Exige calificada, entre otras: cambiar la destinación de bienes comunes, una extraordinaria que supere cuatro veces las expensas mensuales necesarias, gastos distintos de los necesarios, dar un bien común al uso exclusivo de una unidad, y la reconstrucción. **Ninguna decisión puede exigir más del 70 %**, salvo la extinción de la propiedad horizontal — y las mayorías superiores que ponga un reglamento **se tienen por no escritas**. El parágrafo del mismo artículo añade *dónde* puede tomarse la decisión, no solo con cuántos votos: ver RN-94. | `dominio/reglas.ts` (`resultadoVotacion`, `mayoriaDelPunto`) |
| RN-92 | **La forma de asistir no cambia lo que pesa la unidad** (Mary, 2026-09-10: *«la asistencia virtual pesa igual que la presencial»*). No es solo criterio de producto: la **Ley 675 art. 42** admite la reunión no presencial *«de conformidad con **el quórum requerido para el respectivo caso**»* —el mismo quórum, no uno propio— y el **Decreto 398 de 2020, art. 1** lo escribe para las mixtas: *«Las disposiciones legales y estatutarias sobre convocatoria, quórum y mayorías de las reuniones presenciales serán igualmente aplicables a las reuniones no presenciales… y a las reuniones mixtas»*. Por eso `resumenAsistencia.coeficiente` es **una sola suma**. El reparto presencial/virtual se sigue llevando **para el acta** (art. 47 exige decir quién asistió y en qué calidad), no porque una forma cuente menos. La distinción que sí cambia si suma no es dónde estaba la persona sino **si es propietario o apoderado** (RN-51, RN-30). **Ojo al límite:** que la asistencia pese igual no significa que la sesión pueda decidirlo todo — las decisiones de mayoría calificada no se pueden tomar en reunión no presencial (RN-94). | `dominio/reglas.ts` (`resumenAsistencia`) + `componentes/HojaActa.tsx` |
| RN-93 | **La comisión verificadora del acta es opcional** (Mary, 2026-09-10: *«déjala como una opción para que el administrador seleccione, a veces hay revisión»*). La **Ley 675 no la exige**: el art. 47 pide presidente y secretario y no menciona ninguna comisión — la designa la asamblea o la impone el reglamento. De ahí la forma: `Acta.verificadores` es **una lista que puede estar vacía**. Vacía, el acta se aprueba como siempre; con gente, no se aprueba hasta que **todos** revisen. Y **una revisión vale sobre el texto que se revisó**: si el acta se edita después, esa revisión queda sin efecto y hay que volver a pedirla — lo contrario permitiría recoger las firmas y cambiar el texto después, que es el fraude que una comisión existe para impedir. **No se borra nada** (RN-61): la revisión queda con su fecha y el acta muestra que el texto cambió luego. El estado que se muestra —`borrador`, `en_verificacion`, `aprobada`— **se deriva, no se guarda**. | `dominio/reglas.ts` (`actaTieneComision`, `verificacionVigente`, `verificadoresPendientes`, `actaVerificada`, `estadoActa`) |
| RN-94 | **Hay decisiones que una sesión no puede tomar, aunque las vote** (Ley 675 art. 46, parágrafo, verificado el 2026-09-10): *«Las decisiones previstas en este artículo **no podrán tomarse en reuniones no presenciales**, ni en reuniones de segunda convocatoria, salvo que en este último caso se obtenga la mayoría exigida por esta ley»*. Y el mismo artículo cierra: lo adoptado en contravención suya es **absolutamente nulo**. No es un umbral más alto, es una **puerta cerrada**: la copropiedad puede reunirse por Zoom para todo (art. 42, RN-92) menos para esto. **La segunda convocatoria no se bloquea** — la ley la admite si aun así se obtiene el 70 %, que es lo que `resultadoVotacion` ya exige sin relajarlo nunca. **La mixta se trata como no presencial, y eso es una deducción, no una cita**: el art. 46 dice «no presenciales» y en 2001 no existía la mixta; quien la trae al caso es el Decreto 398 de 2020 art. 1 («las reglas relativas a las reuniones no presenciales serán igualmente aplicables a las reuniones mixtas»). Se toma el camino conservador —restringe, no habilita— y queda como pregunta para el abogado (§3 bis). Se comprueba en las dos caras y **en el repositorio**, no solo en el botón (T-16), y el acta lo deja por escrito en vez de reportar una aprobación nula. | `dominio/reglas.ts` (`decisionAdmisibleEnLaSesion`) + `datos/repositorio.ts` (`emitirVoto`) + `componentes/HojaActa.tsx` |
| RN-95 | **La comisión verificadora tiene un plazo máximo, y lo fija el administrador** (Mary, 2026-09-17: *«para la revisión del acta debe existir un plazo máximo que lo define el administrador»*). Sin plazo, un solo miembro que no revise deja el acta en borrador para siempre, y con ella las decisiones de la asamblea: la comisión existe para garantizar el acta, no para bloquearla. El plazo es **por acta, no por reglamento** —se escribe al designar la comisión—, y alrededor de lo que el administrador decide hay dos cosas fijas: **no puede pasar del término del art. 47** (`limiteVerificacion`), porque el acta tiene que estar a disposición en esos veinte días hábiles con o sin revisión, y **con comisión el plazo es obligatorio**: sin él no se aprueba. Mientras corre, el acta espera a todos (RN-93). **Vencido, las revisiones que faltan dejan de detenerla y ya no se registran** —un plazo máximo que admite revisiones después no es máximo—, y **no se borra nada** (RN-61): la hoja dice quién no revisó dentro del plazo y hasta cuándo lo tuvo. El estado se sigue derivando: con el plazo vencido el acta vuelve a `borrador`, no a `en_verificacion`. Se exige **en el repositorio** (T-16), no solo en el `max` del campo de fecha. | `dominio/reglas.ts` (`motivoPlazoComisionInvalido`, `comisionVencida`, `verificadoresQueBloquean`, `verificadoresFueraDePlazo`) + `datos/repositorio.ts` (`designarComisionActa`, `verificarActa`) + `componentes/HojaActa.tsx` |
| RN-96 | **El poder que el propietario envía en foto no vale hasta que la administración lo valide** (Mary, 2026-09-17: que lo envíe *«adjuntando una foto del documento»*). Es la tercera puerta, y se distingue de las otras dos en **quién vio el papel**: en CU-A-19 lo tuvo el administrador en la mano; en CU-R-23 no hay papel porque respalda la sesión; aquí lo vio el propietario, y lo que hace válido un poder en papel es que la administración lo vea. De ahí las tres consecuencias: nace `esperando` y **mientras espera la unidad no está representada** —vota su propietario como si el poder no existiera—; **ocupa el lugar** aunque no valga —no se admite otro poder para esa unidad hasta retirarlo, o serían dos representantes en cola—; y **el rechazo lleva motivo siempre** y se conserva (RN-61): el propietario tiene que saber qué corregir y el expediente por qué no valió. Al validar se vuelve a comprobar que no llegó otro por la puerta de papel. **Foto, no PDF**: no bloquea nada (ADR-0009). Se exige en el repositorio (T-16). | `dominio/reglas.ts` (`poderVigente`, `poderEnCurso`, `poderEnCursoDeUnidad`, `ultimoPoderRechazadoDeUnidad`) + `datos/repositorio.ts` (`enviarPoderEnPapel`, `validarPoder`, `rechazarPoder`) |
| RN-97 | **La marca «No obligatorio»: el administrador exime a una persona de adjuntar la foto y el documento** (equipo, 2026-09-17: *«una opción para el administrador que le permita colocarle una marca para que un propietario, arrendatario o visitante que no quiera adjuntar la foto y/o el documento de identidad no lo haga… el usuario ingresa con la contraseña que le asigna Idiky cuando el administrador o propietario lo crea»*). Es una **excepción a RN-57**, y por eso tiene tres límites: la pone **solo el administrador** —al crear el registro o después, desde su consola—; se pone **sobre un registro concreto**, nunca sobre la copropiedad, que dejaría RN-57 sin efecto; y **queda escrito quién la puso y cuándo**, porque aliviar el requisito es una decisión que alguien tomó. Con la marca el registro **no espera nada de la persona**: pasa directo a la autorización de quien lo creó (RN-59) y se autoriza sin fotos. **La persona entra con el código que Idiky le asignó al crearla** (`codigo`): autorizado el registro, ese código sirve para activar su cuenta en vez del código de un solo uso. Al **visitante no le aplica**: ya no lleva fotos (RN-57). La marca se quita mientras el registro está en curso; si no trajo fotos, vuelve a esperarlas. Se exige en el repositorio (T-16). | `dominio/reglas.ts` (`soportesCompletos`, `sinSoportesPorMarca`, `admiteMarcaNoObligatorio`, `codigoDeRegistroValido`) + `datos/repositorio.ts` (`crearRegistroPersona`, `marcarSoportesNoObligatorios`) + `features/auth/ActivarPage.tsx` |
...
| `codigo` | Lo que la persona escribe, junto con su documento, para abrir su registro y adjuntar (RN-58). Con la marca de abajo, es también **la clave con la que activa su cuenta** (RN-97) |
| `soportesNoObligatorios` | La marca **«No obligatorio»** (RN-97): quién la puso y cuándo. Solo el administrador; solo sobre un registro concreto. Con ella el registro no espera fotos y pasa directo a autorizar |
...
| RN-167 | **Se ofrece solo el proveedor del correo registrado**, no todos: Gmail → «Entrar con Google»; Hotmail, Outlook, Live o MSN → «Entrar con Microsoft»; Yahoo, Ymail o Rocketmail → «Entrar con Yahoo»; cualquier otro dominio → solo el código por SMS. Se decide por el dominio del correo, sin consultar nada. | `reglas.ts` (`proveedorDelCorreo`) |
