...
| **Poder** | Documento con el que un propietario autoriza a otra persona a representar su unidad en una asamblea. **Tres puertas** (las dos primeras, Mary 2026-09-10): el de papel —firmado fuera de la app— que el administrador registra con el soporte adjunto, el que **el propietario otorga desde su app**, ya autenticado, y —tercera puerta, 2026-09-17— el de papel que **el propietario envía en foto** y la administración valida (RN-79). No compiten: el de papel tiene que existir siempre. | `Poder` (`origen: 'papel' \| 'app'`, `validacion`) |
...
| **Reunión no presencial / mixta** | La asamblea que ocurre por medio virtual (Zoom, Meet) o mezclando salón y conexión. **Vale igual que la presencial y con el mismo quórum**: Ley 675 art. 42 y Decreto 398 de 2020 art. 1 (RN-75). Idiky **no transmite**: enlaza la herramienta que la copropiedad ya use (ADR-0007). | `Asamblea.modalidad`, `Asistencia.forma` |
| **Comisión verificadora** | Quienes la asamblea designa para revisar el acta antes de publicarse. **La ley no la exige** —el art. 47 solo pide presidente y secretario— así que en Idiky es **opcional**: si no hubo revisión, el acta se aprueba directo (RN-76). Cuando la hay, tiene **un plazo máximo que fija el administrador**, dentro del término del art. 47; vencido, el acta no la espera más (RN-78). | `Acta.verificadores`, `Acta.verificaciones`, `Acta.limiteComision` |
| **No obligatorio** (marca) | Marca que **solo el administrador** pone sobre un registro para que esa persona no tenga que adjuntar la foto ni el documento (RN-80). El registro se autoriza sin soportes y la persona entra con el código que Idiky le asignó. Queda quién la puso y cuándo. | `RegistroPersona.soportesNoObligatorios` |
